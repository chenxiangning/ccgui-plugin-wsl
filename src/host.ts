...
export function copy(locale: string): Copy {
  if (locale.startsWith("zh")) {
    return {
      ...
      portLabel: "端口",
      userLabel: "用户",
      hostFormHint: "需要本机对宿主已配置 SSH key 认证（插件经 ssh 非交互连接,不支持密码登录）。",
  ...
  return {
    ...
    remoteDesc: "Click 【Connect】 to probe remote 【distros】 and installed 【engines】; the distro panel expands automatically.",
    addrLabel: "Host",
    portLabel: "Port",
    userLabel: "User",
    hostFormHint:
      "SSH key authentication to the host must already be configured (the plugin connects non-interactively via ssh; password login is not supported).",
    ...
    probeHint: "Only in-distro installs count (login-shell PATH incl. ~/.local/bin); /mnt/* (Windows interop) paths are excluded",
    probeDesc: "Detected = the 【CLI】 can be invoked inside the distro; resolved via login-shell PATH.",
    available: "Available",
    notFound: "Not found",
  };
}
